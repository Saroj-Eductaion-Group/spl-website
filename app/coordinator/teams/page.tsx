...
export default function CoordinatorTeams() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="font-headline font-black text-3xl italic uppercase tracking-tighter text-[#e4e1e9]">District <span className="text-[#ffd700]">Teams</span></h1>
          ...
          <div className="bg-[#131318] border border-[#444650]/30 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-6 border-b border-[#444650]/20 sticky top-0 bg-[#131318]">
              <div>
                ...
                <p className="text-xs text-[#c4c6d0]/60 mt-1">{viewTeam.registrationId} • {viewTeam.schoolCollege}</p>
              </div>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4 text-sm bg-[#0b0b0f] border border-[#444650]/20 p-4">
              ...
              {viewTeam.status === 'PENDING' && (
                <div className="flex gap-3 pt-2 border-t border-[#444650]/20">
                  <button onClick={() => updateStatus(viewTeam.id, 'APPROVED')} className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white font-headline font-black uppercase tracking-tight py-2.5 flex items-center justify-center gap-2 transition-colors">
                    <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>check_circle</span> Approve Team
                  </button>
                  <button onClick={() => updateStatus(viewTeam.id, 'REJECTED')} className="flex-1 bg-red-500 hover:bg-red-600 text-white font-headline font-black uppercase tracking-tight py-2.5 flex items-center justify-center gap-2 transition-colors">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            ...
            <tbody className="divide-y divide-[#444650]/10">
                ...
                <tr key={team.id} className="hover:bg-[#1c1c21] transition-colors">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                      ...
                      {team.status === 'PENDING' && (
                        <>
                          <button onClick={() => updateStatus(team.id, 'APPROVED')} className="text-emerald-400/60 hover:text-emerald-400 transition-colors" title="Approve">
                            <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>check_circle</span>
                          </button>
                          <button onClick={() => updateStatus(team.id, 'REJECTED')} className="text-red-400/60 hover:text-red-400 transition-colors" title="Reject">
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-12 text-center text-[#c4c6d0]/40 font-headline uppercase tracking-widest text-sm">No teams found</td></tr>
              )}
            </tbody>
          </table>
        </div>
